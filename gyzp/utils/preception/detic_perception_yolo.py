...
class DeticPerception(PerceptionModule):
    ...
    # @cyw
    def combine_results(self,pred,yolo_pred,height,width,overwrite):
        '''将detic的预测结果和yolo的预测结果结合起来
            Arguments:
                pred: detic 预测结果
                yolo_pred: yolo 预测结果
                height,width: rgb size
                overwrite: True: 使用yolo预测结果覆盖detic预测结果，否则，直接合并
        '''
        ...
        if yolo_pred.masks is not None:
            ...
            # 如果detic里面有，则覆盖，否则，添加
            
        
        if overwrite:
            # 用yolo的结果覆盖detic的结果（大物体都用yolo的）
            yolo_names = yolo_pred.names.values()
        else:
            # 用yolo的结果补充detic的结果（yolo识别出来就用yolo的，否则还用detic的）
            yolo_names = [yolo_pred.names[i] for  i in yolo_class_idcs]
        
        yolo_names = [yolo_name for yolo_name in yolo_names if yolo_name in self.metadata.thing_classes]

        # for id,class_id in enumerate(class_idcs):
        #     if self.metadata.thing_classes[class_id] in yolo_names:
        #         masks[]

        #         yolo_masks = np.concatenate((yolo_masks,masks[id:id+1,:,:]),axis=0)
        #         yolo_class_idcs = np.append(yolo_class_idcs,[class_id],axis=0)
        #         yolo_scores = np.append(yolo_scores,[scores[id]],axis=0)
             
        return yolo_masks, yolo_class_idcs, yolo_scores
    ...
    def predict(
        ...
        Arguments:
            ...
            overwrite: if true 只用detic的小物体识别结果，else 用detic补充yolo的识别
        ...
        if self.yolo_name2id is None:
            self.yolo_name2id = {name:id for id,name in yolo_pred.names.items()}
        ...
        masks, class_idcs, scores = self.combine_results(pred,yolo_pred,height,width,overwrite)
