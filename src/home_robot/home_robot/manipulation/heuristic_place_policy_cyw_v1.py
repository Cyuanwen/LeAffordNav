    ...
    def forward(self, obs: Observations, vis_inputs: Optional[Dict] = None):
        ...
        if self.timestep == 0:
            ...
            if found is None:
                ...
                self.t_go_to_place = self.total_turn_and_forward_steps + 2 # 再加一步角度调整
        ...
        # elif self.timestep == self.t_go_to_top+1:
        #     if self.placement_voxel is not None:
        #         center_voxel_trans = np.array(
        #             [
        #                 self.placement_voxel[1],
        #                 self.placement_voxel[2],
        ...
        #         delta_heading = get_angle_to_pos(center_voxel_trans)
        #         # Must return commands in radians and meters
        #         action = ContinuousNavigationAction([0, 0, delta_heading])
        #     else:
        ...
        # 这段代码有bug，使得可能转了两个角度
            ...
            delta_arm_lift = placement_height - current_arm_lift # 如果为正，是不是说明机器人够不到？
            # # # @cyw
