...
class ObjectNavAgent(Agent):
            ...
            other_inputs = [
                {
                    ...
                    "start_recep": self.semantic_map.get_semantic_map_obj(e,start_recep_goal_category[e][0]),
                    "object":self.semantic_map.get_semantic_map_obj(e,object_goal_category[e][0])
                }
                for e in range(self.num_environments)
            ]
            vis_inputs = [
    ...
    # @cyw
    def towards2goal(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        '''朝向指定点,goal point通过 调用 navigation_agent semantic_map 的 set_goal方法设置
            ref: 上面的nav2goal()
            NOTE 地图的坐标系是在不断变化
        '''
        if self.get_timing:
