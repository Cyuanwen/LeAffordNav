...
"""
copy from src/home_robot_sim/home_robot_sim/env/habitat_ovmm_env/habitat_ovmm_env.py
and modified to adapt with rearrange task
"""
from ast import Str
...
class HabitatOpenVocabManipEnv(HabitatEnv):
    ...
    def __init__(self, habitat_env: habitat.core.env.Env, config, dataset):
        ...
        if "drawer" in config.habitat.dataset.data_path:
            self.target = ["drawer"]
        elif "fridge" in config.habitat.dataset.data_path:
            self.target = ["fridge"]
        self._obj_name_to_id_mapping = {name:idx for idx,name in enumerate(self.target)}
    ...
    def _preprocess_obs(
        ...
        obs = home_robot.core.interfaces.Rearrange_Observations(
            ...
            task_observations={
                ...
                "start_recep_name": self.target[0],
                "place_recep_name":None,
                "object_name":None,
            },
