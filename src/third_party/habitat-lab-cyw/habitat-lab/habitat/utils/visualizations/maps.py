...
def draw_agent(
    ...
    # 旋转角为正，表示逆时针旋转。默认情况下是绕着图片的竖直方向旋转
    # https://docs.scipy.org/doc/scipy/reference/generated/scipy.ndimage.rotate.html
    # AGENT_SPRITE 一开始的朝向为 y轴负方向
...
def draw_agent_by_rot_image(
    ...
    Returns:
        The modified background image. This operation is in place.
        由 draw_agent 修改而来，通过旋转图像获得agent_image
    """
    ...
    rotated_image = scipy.ndimage.interpolation.rotate(
        image, -agent_rotation * 180 / np.pi
    )
    rotated_agent = AGENT_SPRITE
    # 旋转角为正，表示逆时针旋转。默认情况下是绕着图片的竖直方向旋转
    # https://docs.scipy.org/doc/scipy/reference/generated/scipy.ndimage.rotate.html
    # AGENT_SPRITE 一开始的朝向为 y轴负方向
    # Rescale because rotation may result in larger image than original, but
    ...
    utils.paste_overlapping_image(rotated_image, resized_agent, agent_center_coord)
...
# @cyw
def colorize_draw_agent_and_fit_to_height_test(
    ...
    # 将图像旋转 - map_agent_angle 度
    top_down_map = rotate_matrix_numbers(
        matrix= top_down_map,
...
# @cyw
def rotate_matrix_numbers(matrix, angle, center,dsize=None):
    ...
    # 创建旋转矩阵
    rotation_point = [center[1],center[0]]
    rotation_matrix = cv2.getRotationMatrix2D(rotation_point, angle, 1.0)
    
    # 进行图像旋转
    rotated_image = cv2.warpAffine(image, rotation_matrix, dsize, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
