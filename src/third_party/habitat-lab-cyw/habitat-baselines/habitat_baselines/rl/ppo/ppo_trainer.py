...
class PPOTrainer(BaseRLTrainer):
    ...
    def _init_train(self, resume_state=None):
        ...
        # @cyw
        if show_image:         
            cv2.imshow("a",cv2.cvtColor(observations[0]['third_rgb'],cv2.COLOR_BGR2RGB))
            cv2.waitKey(0)
    ...
    def _compute_actions_and_step_envs(self, buffer_index: int = 0):
        ...
        with g_timer.avg_time("trainer.sample_action"), inference_mode():
            ...
            # @cyw
            if show_image:         
                cv2.imshow("a",cv2.cvtColor(step_batch["observations"]['third_rgb'][0].cpu().numpy(),cv2.COLOR_BGR2RGB))
                cv2.waitKey(0)
    ...
    def _eval_checkpoint(
        ...
        if self.config.habitat_baselines.eval.should_load_ckpt:
            ...
            # @gyzp 由于加载baseline无此字段，但此字段无关紧要，因此通过try except处理，忽略此错误
            try:
        ...
        if len(self.config.habitat_baselines.eval.video_option) > 0:
            ...
            rgb_frames: List[List[np.ndarray]] = [
                [
                    observations_to_image(
                        {k: v[env_idx] for k, v in batch.items()}, {} # TODO @gyzp
                    )
                ]
