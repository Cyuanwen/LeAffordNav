...
def select_waypoint(hfov,data_dir):
    '''选择waypoint，条件见最顶端, 将选择的waypoint加到 hdf5文件中
    ...
    # h5py_file = h5py.File(os.path.join(data_dir,"data_out_reshaped.hdf5"),"r+")
    '''
    ...
    for scene_ep_data in tqdm(pkl_data):
        ...
        for recep_data in scene_ep_data["skill_waypoint_data"]:
            ...
            # scene_ep_recep_group = h5py_file[f"/scene_{scene_id}/ep_{episode_id}/{recep_position}"]
            ...
            for waypoint_index, recep_pos in enumerate(recep_relative_pos):
                ...
                if success:
                    ...
                    another_recep = 0
                elif len(viewpoint_relative_pos[waypoint_index])!=0:
                    ...
                    another_recep = 1
                target_point_s.append(target_point)
                success_s.append(success)
                another_recep_s.append(another_recep)
            target_point_s = np.stack(target_point_s,axis=0)
            success_s = np.stack(success_s,axis=0)
            another_recep_s = np.stack(another_recep_s,axis=0)
            # scene_ep_recep_group.create_dataset(name="target_point_s",data=target_point_s)
            # scene_ep_recep_group.create_dataset(name="success_s",data=success_s)
            # scene_ep_recep_group.create_dataset(name="another_recep",data=another_recep_s)
            # print(f"{scene_ep_recep_group['target_point_s']}")
            # if 'target_point_s' in scene_ep_recep_group:
            #     scene_ep_recep_group.__delitem__('target_point_s')
            # scene_ep_recep_group.create_dataset(name="target_point_s",data=target_point_s)
