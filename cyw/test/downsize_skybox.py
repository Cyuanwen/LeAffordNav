...
def merge(images_path):
  ims = []
  for i in range(1,13):
    image = images_path + "/" + str(i) + ".jpg"
    im = cv2.imread(image)
    ims.append(im)
  newimg = np.concatenate(ims,axis=1)
  cv2.imwrite(images_path+"total.jpg",newimg)
...
def downsizeWithMerge(scan):
  ...
  for pano in pano_ids:
    # @cyw
    if not os.path.exists(skybox_merge_template % (base_dir,scan,pano)):
      ims = []
